...
const MyYieldsPage: React.FC = () => {
  ...
  // 1. Calculate static totals via useMemo (No more setTotalDeposited calls)
const summaryTotals = useMemo(() => {
  if (!wallet.address) return { currentDeposit: 0, totalDeposited: 0, totalWithdrawn: 0, totalEarned: 0 };

  const addresses = isConsolidated 
    ? [...manualAddresses, ...(isMetamaskConnected && metamaskAddress ? [metamaskAddress] : [])]
    : [wallet.address];

  let currentD = 0, totalD = 0, totalW = 0, totalE = 0;

  addresses.forEach(addr => {
    const userData = activityData[addr.toLowerCase()]; // Access directly from stable activityData
    if (userData) {
  ...
  return { currentDeposit: currentD, totalDeposited: totalD, totalWithdrawn: totalW, totalEarned: totalE };
}, [wallet.address, isConsolidated, manualAddresses, isMetamaskConnected, metamaskAddress, activityData]);

// 2. Only use State for the "Live" ticking values
useEffect(() => {
    setCurrentEarned(summaryTotals.totalEarned); // Reset live state when base data changes
