...
const MyYieldsPage: React.FC = () => {
  ...
  const activityData = useDepositsAndWithdrawalsStore(
    (state) => state.activityData,
  );
  const getUserActivity = useDepositsAndWithdrawalsStore(
    (state) => state.getUserActivity,
  );
  ...
  const baseFilterMatch = useCallback(
    (asset: Asset) => {
  ...
  const getApyForAsset = useCallback(
    (asset: Asset) => {
      if (
        asset.protocol &&
  ...
  const getOptimizationDataForAsset = (
    ...
    if (
      bestApy !== null &&
      bestProtocol &&
      bestProtocol !== asset.protocol &&
      bestApy > currentApy
    ) {
      const balanceNum = parseFloat(asset.balance);
      const usdPrice = parseFloat(asset.balanceUsd) / balanceNum;
      const additionalYearlyUsd =
        balanceNum * ((bestApy - currentApy) / 100) * usdPrice;
      return {
        ...
        apyImprovement: parseFloat(
          (((bestApy - currentApy) / currentApy) * 100).toFixed(0),
        ),
      };
    }
    return undefined;
      ...
      {loading ? (
        ...
        <div className={styles.summaryCards}>
          ...
          <div className={styles.summaryCard}>
            ...
            <div className={styles.summaryAmount}>
              ${formatLiveValue(summaryTotals.currentEarned)}
            </div>
            <div className={styles.summarySubtext}>
              Total Earning ${formatLiveValue(summaryTotals.totalEarned)}
            </div>
          </div>
          <div className={styles.summaryCard}>
              ...
              <>
                {activeYieldingAssets.length > 0 ? (
                  ...
                  <NoYieldEmptyState
                    ...
                    onRedirect={() =>
                      navigate(hasAnyBalance ? "/" : "/explore")
                    }
                  />
                )}
              </>
