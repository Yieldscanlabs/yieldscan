...
const SettingsPage: React.FC = () => {
  ...
  const { activeDecimalDigits, setActiveDecimalDigits } =
    useUserPreferencesStore();
  const { wallet, isModalOpen, closeConnectModal } = useWalletConnection();
  ...
  const handleOptionChange = async (value: number) => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/user-details/${address.toLowerCase()}`,
        {
          method: "PATCH",
          ...
          <div className={styles.cardHeader}>
            ...
            <p className={styles.subtext}>
              ...
              {address ? (
                <CopyWrapper
                  text={address}
                  shortenText={shortenAddress(address)}
                />
              ) : (
          ...
          <div className={styles.settingItem}>
            <div className={styles.labelGroup}>
              ...
              <span className={styles.description}>
                Choose how many decimal places to display for currency values
                (Rounded Down).
                <i style={{ color: "orange", marginLeft: "5px" }}>
                  This will reflect throught yieldscan values except apys.
                </i>
              </span>
            </div>
