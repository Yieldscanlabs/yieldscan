...
import {
  MyYieldSkeletonLoader,
  MyYieldSummaryCardsSkeleton,
} from "../components/loaders/MyYieldSkeletonLoader";

// NEW: Import persistent filter logic
import {
  checkIsYieldAssetVisible,
  HARD_MIN_USD,
  useLowValueFilter,
} from "../hooks/useLowValueFilter";
...
const MyYieldsPage: React.FC = () => {
  ...
  const { yieldsPageView: viewType, setYieldsPageView: setViewType } =
  ...
  const uniqueAssets = useMemo(() => {
    ...
    allYieldAssets.forEach((asset) => {
      // FIX: Use checkIsYieldAssetVisible with 'false'.
  ...
  const baseFilterMatch = (asset: Asset) => {
    if (selectedNetwork !== "all" && asset.chainId !== selectedNetwork)
      return false;
    if (
      selectedProtocol !== "all" &&
      asset.protocol?.toLowerCase() !== selectedProtocol.toLowerCase()
    )
      return false;
    if (
      selectedAsset !== "all" &&
      asset.token.toLowerCase() !== selectedAsset.toLowerCase()
    )
      return false;
    if (searchQuery) {
      const matchesToken = asset.token
        .toLowerCase()
        .includes(searchQuery.toLowerCase());
      const matchesProtocol = asset.protocol
        ?.toLowerCase()
        .includes(searchQuery.toLowerCase());
  ...
  const filteredYieldAssets = useMemo(() => {
    return allYieldAssets.filter(baseFilterMatch).map((asset) => {
      ...
      if (userData) {
        ...
        if (chainData) {
          ...
          if (protocolData) {
            ...
            const tokenData =
              protocolData[tokenKey] ||
              protocolData[tokenKey.replace(/^a/i, "")];
            if (tokenData) {
  ...
  const getOptimizationDataForAsset = (
    asset: Asset,
    ...
    if (bestProtocol !== currentProtocol && bestApy > currentApyEstimate) {
      ...
      const betterProtocolName =
        bestProtocol && bestProtocol.toUpperCase() in PROTOCOL_NAMES
          ? PROTOCOL_NAMES[
              bestProtocol.toUpperCase() as keyof typeof PROTOCOL_NAMES
            ]
          : bestProtocol;
      return {
        currentProtocol,
  ...
  const summaryTotals = useMemo(() => {
    ...
    const addresses = isConsolidated
      ? [
          ...manualAddresses,
          ...(isMetamaskConnected && metamaskAddress ? [metamaskAddress] : []),
        ]
      : [wallet.address];
  ...
  // 🟢 NEW: Calculate Consolidated Projected Yields (Daily & Yearly) based on Active Filters
  const yieldProjections = useMemo(() => {
    let daily = 0;
    let yearly = 0;

    // We use filteredYieldAssets so it respects Networks, Protocols, and Search filters
    filteredYieldAssets.forEach((asset) => {
      const apy = getApyForAsset(asset);
      // Use the current value in protocol (Working Capital)
      const balanceUsd = Number(asset.currentBalanceInProtocolUsd || 0);

      if (balanceUsd > 0 && apy > 0) {
        const yearlyYield = balanceUsd * (apy / 100);
        yearly += yearlyYield;
  ...
  // ... (Live Earning Listener comments removed for brevity) ...
  ...
  const globalState = useMemo(() => {
    const hasActiveYields = assets.some(
      (a) => Number(a.currentBalanceInProtocolUsd) > HARD_MIN_USD,
    );
    const hasDormantFunds = assets.some(
      (a) => Number(a.balanceUsd) > HARD_MIN_USD,
    );
    ...
    <div className={styles.container}>
      ...
      <div className={styles.filtersContainer}>
        ...
        <div className={styles.lowAmountFilterContainer}>
          <LowValueFilterCheckbox
            checked={hideLowValues}
            onChange={setHideLowValues}
      ...
      {loading ? (
        ...
        <div className={styles.summaryCards}>
          ...
          {/* 🟢 CHANGED: Replaced 'Total Withdrawn' with 'Daily Yield' (Accumulated/Projected Yield View) */}
          <div className={styles.summaryCard}>
            <div className={styles.summaryTitle}>Daily Yield</div>
            {/* Display Daily Yield as primary metric */}
            <div className={styles.summaryAmount}>
              ${formatNumber(yieldProjections.daily, 2)}
            </div>
            {/* Display Yearly Yield as subtext */}
            <div className={styles.summarySubtext}>
              Yearly Yield ${formatNumber(yieldProjections.yearly, 2)}
            </div>
          </div>
        </div>
      )}
      {isConsolidated ? (
        (() => {
          ...
          return allAddresses.map((address) => {
            ...
            const allWalletAssets =
              assetsByWallet.get(address.toLowerCase()) || [];
            ...
            const maxApy =
              activeYieldingAssets.length > 0
                ? Math.max(
                    ...activeYieldingAssets.map((a) => getApyForAsset(a)),
                  )
                : 0;
            ...
            // 🟢 NEW: Check if this wallet *really* has yield assets regardless of UI filters.
            const rawWalletAssets = allYieldAssets.filter(
              (a) => a.walletAddress?.toLowerCase() === address.toLowerCase(),
            );
            const walletTrulyHasYieldAssets =
              hasAnyYieldAssets(rawWalletAssets);
                  ...
                  <>
                        ...
                        <div className={styles.yieldGrid}>
                          {activeYieldingAssets.map((asset) => (
                            <YieldCard
                              ...
                              asset={asset}
                              optimizationData={getOptimizationDataForAsset(
                                asset,
                              )}
                              onOptimize={() => {}}
                              isHighestYield={
                                maxApy > 0 && getApyForAsset(asset) === maxApy
                              }
                            />
                          ))}
                        </div>
                    ...
                    hasActiveFilters && walletTrulyHasYieldAssets ? (
                      ...
                      <NoYieldEmptyState
                        ...
                        onRedirect={() =>
                          handleRedirect(hasAnyBalance ? "/" : "/explore")
                        }
                      />
                    )}
                  </>
          ...
          {loading ? (
            ...
            (() => {
              ...
              const activeYields = filteredYieldAssets
                .filter(
                  (a) =>
                    Number(a.currentBalanceInProtocolUsd || 0) >= HARD_MIN_USD,
                )
                .sort((a, b) => getApyForAsset(b) - getApyForAsset(a));
              ...
              // 🟢 NEW: Check for filter state in Single View
              const currentAddress = wallet.address?.toLowerCase();
              const rawWalletAssets = allYieldAssets.filter(
                (a) => a.walletAddress?.toLowerCase() === currentAddress,
              );
              const walletTrulyHasYieldAssets =
                hasAnyYieldAssets(rawWalletAssets);
                ...
                <>
                  {activeYields.length > 0 ? (
                    ...
                    <NoYieldEmptyState
                      onRedirect={() =>
                        handleRedirect(
                          globalState.hasDormantFunds ? "/" : "/explore",
                        )
                      }
                      subtext={
...
const NoYieldEmptyState: React.FC<NoYieldEmptyStateProps> = ({
  onRedirect,
  subtext,
  btnText,
}) => {
  return (
    <div className={styles.emptyState}>
      ...
      <button className={styles.exploreButton} onClick={onRedirect}>
