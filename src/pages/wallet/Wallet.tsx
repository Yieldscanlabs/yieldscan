...
function Wallet() {
  ...
  const handleRedirect = (path: string) => {
    const navigationState =
      state?.selectedNetwork !== "all"
        ? { filterNetwork: state?.selectedNetwork }
        : undefined;
  ...
  const enrichAssetsWithApy = (assetList: Asset[]) => {
    return assetList.map((asset) => {
      const { bestApy, bestProtocol } = getBestYield(
        apyData,
        asset.chainId,
        asset.address,
      );
  ...
  const renderAssetView = () => {
    const baseFilterMatch = (asset: Asset) => {
      const matchesNetwork =
        state.selectedNetwork === "all" ||
        Number(asset.chainId) === Number(state.selectedNetwork);
      const matchesAssetFilter =
        state.selectedAssetFilter === "all" ||
        (asset.token &&
          asset.token.toLowerCase() ===
            state.selectedAssetFilter.toLowerCase());
      const matchesSearch =
        state.searchQuery === "" ||
        (asset.token &&
    ...
    const processWalletAssets = (walletAssets: Asset[]) => {
      const filtered = walletAssets.filter(
        (asset) => baseFilterMatch(asset) && shouldShowWalletAsset(asset),
      );
